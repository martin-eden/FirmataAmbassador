...
return
  function(self)
    ...
    -- Framing message:
    if (LastByte == SysexStartCmd) then
      -- Variable-length.
      LastByte = ReadUntil(SysexEndCmd, Result.Data)

      if is_nil(LastByte) then
        -- Sysex not ended.
        Complain('Sysex not ended.')
        return
      end

      Result.Command = Result.Data[1]

      -- Remove Sysex frame bytes from <Data>:
